...
author: Sydney Levine (smlevine@mit.edu), Max Kleiman-Weiner, Laura Schulz, Joshua
  Tenenbaum, & Fiery Cushman
...
### Import Data
data <- read_csv("experiment/pilot_data.csv")
head(data)
...
data_filtered <- data_filtered %>%
  ...
  group_by(condition) %>%
  summarise(m = mean(acceptability01),
            stdev= sd(acceptability01),
            se = stdev/sqrt(n()),
            upper= m+se*1.96,
            lower=m-se*1.96) 
```
...
ggplot(data_filtered, aes(x = condition, y = m)) +
  ...
  ggtitle('Replication of Study 2a') +
